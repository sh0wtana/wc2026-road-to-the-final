<script>
  ...
  function rowClass(team) {
    ...
    if (isPickable)
      return 'text-slate-100 bg-surface hover:bg-surface-hover hover:text-amber-300 cursor-pointer transition-colors'
    return 'text-slate-100 bg-surface cursor-default'
  }
</script>

<div
  class="rounded border text-sm w-full overflow-hidden divide-y divide-slate-700 flex flex-col
  {isFinal && isDone
    ? 'border-amber-400 shadow-[0_0_16px_theme(colors.amber.400/35%)]'
    : ''}
...
>
  ...
  >
    {#if homeTeam}
      <span>{homeTeam.flag}</span><span class="truncate font-semibold uppercase"
        >{homeTeam.name}</span
      >
    {:else}
      <span>—</span>
  ...
  >
    {#if awayTeam}
      <span>{awayTeam.flag}</span><span class="truncate font-semibold uppercase"
        >{awayTeam.name}</span
      >
    {:else}
      <span>—</span>
