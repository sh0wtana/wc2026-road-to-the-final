...
<div class="bg-surface border border-border-subtle rounded-lg p-2">
  <div
    class="text-xs font-bold text-amber-400 text-center mb-2 uppercase tracking-widest"
  >
    GROUP {group}
  </div>
  <div
  ...
  >
    {#each teams as team, i (team.id)}
      <div
        class="relative group/row flex items-center gap-1 bg-surface-raised hover:bg-surface-hover rounded px-1.5 py-1 cursor-grab active:cursor-grabbing select-none text-sm transition-colors"
      >
        <span
          class="text-sm w-4 text-center leading-none {i === 3
        ...
        <!-- Tooltip -->
        <div
          class="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 z-50
