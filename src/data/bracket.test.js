...
describe('R32_MATCHES', () => {
  ...
  it('away slots are 2nd-place or third-place slots', () => {
    for (const m of R32_MATCHES) {
      const isSecond = m.away.length === 2 && m.away[0] === '2' && VALID_GROUPS.has(m.away[1])
      const isThird  = m.away.startsWith('3') && [...m.away.slice(1)].every(g => VALID_GROUPS.has(g))
...
describe('THIRD_PLACE_SLOTS', () => {
  ...
  it('matches the away slots in R32_MATCHES exactly', () => {
    const thirdAways = R32_MATCHES.map(m => m.away).filter(a => a.startsWith('3')).sort()
...
describe('feed completeness', () => {
  it('every R32 match feeds into exactly one R16 match', () => {
    ...
    for (const id of R32_IDS) {
      expect(r16Feeders.filter(f => f === id), `${id} in R16_FEED`).toHaveLength(1)
    }
  })
  ...
  it('every R16 match feeds into exactly one QF match', () => {
    ...
    for (const id of R16_IDS) {
      expect(qfFeeders.filter(f => f === id), `${id} in QF_FEED`).toHaveLength(1)
    }
  })
  ...
  it('every QF match feeds into exactly one SF match', () => {
    ...
    for (const id of QF_IDS) {
      expect(sfFeeders.filter(f => f === id), `${id} in SF_FEED`).toHaveLength(1)
    }
  })
